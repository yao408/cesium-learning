  ...
  <div class="factory-detail-page">
    ...
    <div class="info-panel">
      <div class="panel-header">
        ...
        <span class="factory-type">{{ factoryInfo.type }}</span>
      </div>
      <div class="panel-body">
        <p class="description">{{ factoryInfo.description }}</p>
        <div class="position-info">
          <span class="label">位置:</span>
          <span class="value">{{ factoryInfo.position?.lat?.toFixed(4) }}, {{ factoryInfo.position?.lng?.toFixed(4) }}</span>
        </div>
      </div>
      <div class="panel-footer">
        <button class="btn-back" @click="goBack">← 返回地图</button>
        <button class="btn-reset" @click="resetCamera">重置视角</button>
      </div>
    </div>
    
    <!-- 操作提示 -->
    <div class="controls-hint">
      <span>🖱️ 左键旋转 | 右键平移 | 滚轮缩放</span>
    </div>
  </div>
</template>
...
onMounted(() => {
  ...
  const info = getFactoryById(factoryId)
  
  if (!info) {
    console.error('工厂不存在:', factoryId)
    return
  }
  
  factoryInfo.value = info
  initThreeJS(info.modelConfig)
})
...
function initThreeJS(modelConfig) {
